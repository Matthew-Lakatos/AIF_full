...
class DAIF_Tschantz(nn.Module):
    ...
    # from its own free-energy terms, not on `reward`. Comparing its "Reward"
    # column against PPO/EmotionPPO (USES_ENV_REWARD=True) is not a like-for-
    # like comparison of the same optimization target.
    USES_ENV_REWARD = False
    REWARD_SOURCE = "self-supervised pseudo-reward (free-energy proxy), not raw environment reward"
