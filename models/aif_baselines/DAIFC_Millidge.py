...
class DAIFC_Millidge(nn.Module):
    ...
    # Its actor-critic is trained on a self-supervised pseudo-reward derived
    # from its own prediction/state-cost terms, not on `reward`. Comparing its
    # "Reward" column against PPO/EmotionPPO (USES_ENV_REWARD=True) is not a
    # like-for-like comparison of the same optimization target.
    USES_ENV_REWARD = False
    REWARD_SOURCE = "self-supervised pseudo-reward (prediction/state-cost proxy), not raw environment reward"
