...
class FEAC_Friston(nn.Module):
    ...
    # This agent never receives the environment's reward (see bug report #1).
    # Its critic target is `pseudo_rew = -((s_next - s_prev) ** 2).mean()` --
    # i.e. it is rewarded for keeping its own latent representation from
    # changing between steps, with no connection to the task at all. Its low
    # task reward is not necessarily evidence of a worse implementation; it
    # may simply never have been trying to maximize task reward. Comparing
    # its "Reward" column against PPO/EmotionPPO (USES_ENV_REWARD=True) is not
    # a like-for-like comparison of the same optimization target.
    USES_ENV_REWARD = False
    REWARD_SOURCE = "self-supervised pseudo-reward (negative encoder-output drift), not raw environment reward"
