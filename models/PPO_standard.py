...
DESIGN NOTE (bug report #11): this is intentionally NOT textbook batched PPO.
Standard PPO collects a rollout batch and runs several epochs of minibatch SGD
over it; here, every single environment step gets its own from-scratch
`k_epochs`-epoch PPO-clip update on a batch of size 1 (`advantage`/`old_logprob`
are fixed from before those epochs). This matches the AIF-family agents'
step-by-step update cadence for a fairer cadence-wise comparison, but it means
results here shouldn't be read as "vanilla batched PPO with GAE" -- there is no
batching and no GAE. Keep this in mind when writing up the comparison.
"""
...
class PPOActorCritic(nn.Module):
    # This agent is trained directly on the environment's reward (see
    # bug report #1 -- the AIF-family baselines below are NOT).
    USES_ENV_REWARD = True
    REWARD_SOURCE = "environment reward"
