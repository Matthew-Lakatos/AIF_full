...
class EFEPlanner:
    ...
    def __call__(self, world_model, ensemble, policy_dist, h_start, z_start, preferred_obs, s_start=None):
        """
        ...
        Args:
            ...
            s_start: [B, slow_dim] tensor representing the current slow-timescale
                     context latent, or None if the world model has no slow
                     latent (`world_model.slow_dim == 0`) or the caller has no
                     context to provide. Rollouts start from this value.

                     FIX (bug report #3): this parameter did not exist before.
                     Rollouts previously always started the slow context from
                     zero, regardless of the agent's actual belief, because
                     this planner looked for a `world_model.s` attribute that
                     the real RSSMWorldModel never defines (belief state lives
                     on the agent, not the world model) -- so the lookup
                     always fell through to a zero tensor. Callers should now
                     pass the agent's real current slow latent explicitly.
        ...
        Notes:
            - Whether gradients flow to h_start/z_start/s_start is entirely the
              caller's choice (e.g. detach them, or not); this planner does not
              detach or require grad on them itself. Gradients to the policy
              flow through `policy_dist` (via `.rsample()`), independent of
              whatever h_start/z_start/s_start's grad status is.
        ...
        for p in range(self.num_particles):
            ...
            # Start the slow-context rollout from the caller-provided s_start.
            # Only fall back to zeros if the caller genuinely has no context to
            # give us (s_start is None) -- e.g. slow_dim == 0, or a caller that
