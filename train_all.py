...
def train_single(
    ...
    for ep in range(episodes):
        ...
        # FIX (bug report #4): AffectiveTutor and LongHaulMission (among others)
        # can end an episode early -- before the fixed-position blackout window
        # is ever reached -- if a per-environment failure condition triggers
        # (e.g. frustration crossing a threshold, or a system's integrity
        # hitting zero). When that happens, BlackoutReward/RecoveryReward for
        # that episode are simply missing (NaN), which is indistinguishable
        # from "blackout wasn't enabled for this run" unless we also record
        # *why* the data is missing. Recording episode length and whether the
        # blackout window was actually reached turns that invisible NaN into
        # a directly reportable "early-termination rate" per model (see
        # aggregate_blackout_pooled below) -- an agent that never reaches the
        # blackout window is a materially different (and more informative)
        # outcome than "no blackout data available".
        reached_blackout_window = float("nan")
        if (
            bool(getattr(env, "blackout_enabled", False))
            and getattr(env, "blackout_start_step", None) is not None
            and getattr(env, "blackout_start_step", -1) >= 0
        ):
            reached_blackout_window = 1.0 if step_count >= env.blackout_start_step else 0.0
        ...
        row = {
            ...
            # See bug report #4: EpisodeLength + ReachedBlackoutWindow make
            # early-termination-driven survivorship bias visible instead of
            # silently indistinguishable from "no blackout data available".
            "EpisodeLength": step_count,
            "ReachedBlackoutWindow": reached_blackout_window,
        }
...
def aggregate_blackout_pooled(df_all: pd.DataFrame, last_n_episodes: int = 50) -> dict:
    ...
    for out_name, col in cols_to_pool.items():
        ...
        # STATISTICAL FIX (bug report #7): "_pooled_sem" above treats every
        # episode from every seed as an independent replicate. Episodes from
        # the same seed share one trained network and are NOT independent
        # draws with respect to "how would a *new* random seed behave" -- so
        # pooling them into one large n inflates n and understates the true
        # uncertainty on between-seed generalization. The correct unit of
        # independent replication here is the SEED (3-5 of them), not the
        # episode. We collapse to one mean per seed first, then compute the
        # mean/SEM across THOSE few, genuinely-independent numbers. Use
        # "_between_seed_sem" (not "_pooled_sem") for any claim of the form
        # "would this reproduce with a new seed" -- e.g. any significance
        # test. The "_pooled_*" statistics above remain a legitimate
        # descriptive summary of this specific set of trained seeds; they are
        # kept, not replaced, for backward compatibility.
        if "Seed" in window_df.columns:
            per_seed_means = window_df.groupby("Seed")[col].mean().dropna().to_numpy()
            n_seeds = int(len(per_seed_means))
            seed_mean = float(per_seed_means.mean()) if n_seeds > 0 else float("nan")
            seed_std = float(per_seed_means.std(ddof=1)) if n_seeds > 1 else float("nan")
            seed_sem = float(seed_std / np.sqrt(n_seeds)) if n_seeds > 1 else float("nan")
            out[f"{out_name}_n_seeds"] = n_seeds
            out[f"{out_name}_between_seed_mean"] = seed_mean
            out[f"{out_name}_between_seed_sem"] = seed_sem

    # FIX (bug report #4): surface how often episodes never even reached the
    # blackout window, per model, rather than leaving it as an invisible NaN
    # indistinguishable from "no blackout data available". See EpisodeLength/
    # ReachedBlackoutWindow in train_single().
    if "ReachedBlackoutWindow" in window_df.columns:
        reached = window_df["ReachedBlackoutWindow"].dropna().to_numpy()
        if len(reached) > 0:
            out["EarlyTermination_BeforeBlackout_rate"] = float(1.0 - reached.mean())
            out["ReachedBlackoutWindow_n_episodes"] = int(len(reached))
...
def main():
    ...
    for agent_name in args.agents:
        ...
        # FIX (bug report #1): DAIF/PCRL/FEAC/DAIFC are never trained on the
        # environment's reward at all (they optimize a self-supervised
        # pseudo-reward instead); AIF optimizes preferred_obs-based risk, not
        # raw reward either. Only PPO/EmotionPPO directly optimize the metric
        # in the "Reward" column. This doesn't change any agent's training --
        # it just makes that fact travel with the numbers instead of being
        # discoverable only by reading source code. Read off the agent
        # CLASS (not an instance) since we don't keep one around here.
        agent_cls = AGENTS[agent_name]
        reward_meta = {
            "OptimizesTaskReward": bool(getattr(agent_cls, "USES_ENV_REWARD", True)),
            "RewardSource": getattr(agent_cls, "REWARD_SOURCE", "environment reward"),
        }

        all_pooled_rows.append({"Model": agent_name, **reward_meta, **pooled})
