...
def make_agent_variant(variant_name, seed, use_context=True):
    ...
    Create agent variants:
      ...
      - 'no-persistence': set agent.lambda_slow to 0 (disables the slow-latent
        persistence penalty's contribution to the training loss)
      - 'fixed-average': replace the learned precision controller with a
        constant beta=0.5, so complexity_loss uses a fixed weight instead of
        a learned dynamic one. (This does NOT disable posterior correction --
        `use_posterior_correction` is left at its default for every variant
        here; only the precision controller itself is replaced.)
    """
    ...
    if variant_name == "full":
        ...
        # NOTE (bug report #11): the persistence penalty's weight in the loss
        # is agent.lambda_slow (see `self.lambda_slow * slow_persist` in
        # AIF.update()) -- that's the only line that needs to change here.
        # world_model.lambda_slow is stored on RSSMWorldModel but never read
        # anywhere in rssm_world_model.py, so setting it was dead code that
        # looked load-bearing but wasn't; removed rather than left in as a
        # misleading no-op.
