...
class TransitionEnsemble(nn.Module):
    ...
    def __init__(self, *args, input_dim=None, latent_dim=None, ensemble_size=3, **kwargs):
        ...
        if len(args) >= 2:
            ...
            # FIX (bug report #6): this used to also fall back to
            # kwargs.get('action_dim', None), on the theory that it was
            # "accepting an alternative name" for input_dim. It isn't one --
            # input_dim is the full concatenated width forward() will receive
            # (e.g. latent_dim + action_dim + slow_dim), not action_dim alone.
            # A caller that (mistakenly) passed action_dim= as a keyword would
            # have this silently aliased into input_dim, causing eager
            # construction to build with far too small a width; the only
            # reason nothing broke visibly is that forward()'s width-mismatch
            # check below rebuilds the ensemble from scratch the first time
            # it's actually called with correctly-shaped data -- which defeats
            # eager construction's entire purpose (avoiding a weight reset)
            # and silently throws away one full initialization. If input_dim
            # isn't given explicitly (as a real keyword named input_dim, or a
            # positional arg above), we now just fall through to lazy
            # construction on the first real forward() call instead of
            # guessing at it from an unrelated kwarg.
