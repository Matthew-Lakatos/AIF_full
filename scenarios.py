...
class TransferEntropy(ResearchEnvironment):
    ...
    def preferred_obs(self):
        ...
        FIX (bug report #11): the training loop queries preferred_obs()
        *before* calling step() for this same transition, but step() itself
        increments self.steps first and then computes `goal` from the NEW
        value. That meant preferred_obs() and step() disagreed about the
        goal for exactly one step, right at the flip point (self.steps == 249
        going into the call that makes it 250). Using `self.steps + 1` here
        mirrors what step() will actually use for this same upcoming
        transition, so the two stay in sync.
        """
        goal = 1.0 if (self.steps + 1) < 250 else -1.0
        pref = torch.zeros(self.obs_dim)
        pref[0] = goal
        return pref
