...
def plot_diagnostics(
    ...
    # FIX (bug report #9): this used to do, inside the `for metric in
    # diag_metrics` loop (which is easily 50+ iterations once every phase-
    # suffixed diagnostic is counted):
    #     df = agg_df.sort_values(["Model", "Episode"])   # every iteration
    #     df[f"{metric}_smooth"] = (...)                  # one column at a time
    # Re-sorting the whole frame every iteration was pure waste, and
    # assigning one column at a time onto a frame built by concatenating
    # differently-shaped per-agent data (each of the seven agents logs a
    # different set of diagnostics, so the combined frame ends up with many
    # NaN-filled, differently-typed columns) fragments its memory layout --
    # exactly the `PerformanceWarning: DataFrame is highly fragmented` spam
    # this produced, once per diagnostic metric, every environment/blackout
    # combo. Fix (per pandas' own suggestion in that warning): sort once, and
    # join all the smoothed columns in a single pd.concat instead of N
    # sequential single-column assignments.
    df = agg_df.sort_values(["Model", "Episode"]).copy()
    smoothed_cols = {
        f"{metric}_smooth": df.groupby("Model")[metric].transform(
            lambda x: _smooth(x, window=smooth_window)
        )
